...
const FloatingDockMobile = ({ items, className }: { items: Items[]; className?: string }) => {
              ...
              >
                {item.href && (
                  <Link
                    href={item.href}
                    key={item.title}
                    className="flex h-10 w-10 items-center justify-center rounded-full bg-card shadow-lg border"
                  >
                    <item.icon />
                  </Link>
                )}
...
const FloatingDockDesktop = ({ items, className }: { items: Items[]; className?: string }) => {
  ...
  return (
    <motion.div
      ...
      className={cn(
        'mx-auto hidden h-16 px-4 pb-3 items-end gap-4 rounded-3xl bg-background/25 backdrop-blur-[4px] md:flex shadow-lg border',
        className
      )}
    >
      {items.map(item => (
...
function IconContainer({ mouseX, title, icon: Icon, href, onClick }: Items & { mouseX: MotionValue }) {
        ...
        <TooltipTrigger>
          <Link href={href} aria-label={title}>
            <motion.div
              ...
              className="relative flex aspect-square items-center justify-center rounded-full bg-card border"
            >
